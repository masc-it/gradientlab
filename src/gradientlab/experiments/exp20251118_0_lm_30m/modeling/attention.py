...
class Attention(nn.Module):
    def __init__(
        ...
        num_heads: int,
        use_bias: bool = True,
        qk_ratio: float = 1.0,  # <1.0 squeezes Q/K head dim
        v_ratio: float = 1.0,  # can be 1.0–2.0 to expand V head dim
        ...
        self.out_dim = self.num_heads * self.v_head_dim
    ...
    def forward(
        ...
        x = x.transpose(1, 2).reshape(B, L_Q, self.out_dim)
        x = self.out_proj(x)
        return x
